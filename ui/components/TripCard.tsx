...
import { ChevronDown, ChevronUp, Heart, Stethoscope } from "lucide-react";
...
export function TripCard({ className, map }: TripCardProps) {
  ...
  return (
    <Card className={cn("z-50 bg-white/40 backdrop-blur-md border-none shadow-2xl flex flex-col h-[calc(100vh-12rem)]",
      className,
      minimized ? "h-fit" : "h-[calc(100vh-12rem)]"
    )}>
        <CardHeader className={cn("flex flex-row justify-between items-center bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-t-lg flex-shrink-0",
          ...
          <div className="flex flex-row items-center gap-2">
            ...
            <CardTitle className="text-wrap truncate max-w-full font-bold">
              {selectedTrip?.name || "Health Profile"}
            </CardTitle>
          </div>
          <TripSelect />
        </CardHeader>
